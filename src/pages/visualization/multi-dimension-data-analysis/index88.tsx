...
`
    ...
    const getTodayHeartbeat = async () => {
        const d1 = new Date();
        console.log('getUTCFullYear',d1.getUTCFullYear());
        console.log('getUTCMonth',d1.getUTCMonth());
        console.log('getUTCDate',d1.getUTCDate());
//    where: {id_gte: "2023-2-25-0", id_lt: "2023-2-26-0"}
        setTodayHeartbeatLoading(true);
        const tokensQuery = `
    query MyQuery($id_gte: String,$id_gte: String) {
     heartbeateds(first:1000,where: {blockTimestamp_gte: $blockTimestamp_gte, blockTimestamp_lt: $blockTimestamp_lt}
  ) {
    id
    blockTimestamp
  }
